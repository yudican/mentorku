                ...
                <div class="card-body">
                    <h4 class="card-title text-capitalize">
                        <a href="{{route('dashboard')}}">
                            <span><i class="fas fa-arrow-left mr-3 text-capitalize"></i>sub categories</span>
                        </a>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-capitalize" id="my-modal-title">
                            {{$update_mode ? 'Update' : 'Tambah'}} sub categories</h5>
                    </div>
                    <div class="modal-body">
                        <x-select name="category_id" label="Nama Kategori">
                            <option value="">Select Nama Kategori</option>
                            @foreach ($categories as $category)
                            <option value="{{$category->id}}">{{$category->category_name}}</option>
                            @endforeach
                        </x-select>
                        <x-text-field type="text" name="subcategory_name" label="Nama Sub Kategori" />
                    </div>
